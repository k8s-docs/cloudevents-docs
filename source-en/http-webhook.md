...
"Webhooks" 是一种流行的模式，用于在应用程序之间和通过 HTTP 端点传递通知。
尽管模式的使用非常广泛，但是 Web 钩子还没有正式的定义。
本规范旨在为[CNCF CloudEvents][ce]提供这样一个定义，但被认为在 CloudEvents 范围之外通常可用。
...
["webhook"][webhook]是一种流行的模式，用于在应用程序之间和通过 HTTP 端点传递通知。
使通知可用的应用程序允许其他应用程序注册通知传递到的 HTTP 端点。

该规范通过发送方如何传递通知来定义一个 HTTP 方法、一个用于事件传递的授权模型来保护传递目标，以及一个注册握手来保护发送方不被滥用，以免向任意 HTTP 站点发送大量请求。
