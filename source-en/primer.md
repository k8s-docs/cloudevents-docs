...
本非规范性文档提供了 CloudEvents 规范的概述。
它旨在补充 CloudEvent 规范，为规范开发过程中的历史和设计决策提供额外的背景和见解。
这使得规范本身可以专注于规范的技术细节。
...
1. The [base specification](spec.md) defines an abstract information
   ...
   for what constitutes a CloudEvent. This includes _core attributes_ defined
...
由于安全和策略方面的考虑，可能需要过滤、转换或阻止系统之间的事件传输。
例如，为了防止事件的输入或输出，例如包含敏感信息的事件数据，或者想要禁止发送方和接收方之间的信息流。

通用的事件格式将允许更容易地推理正在传输的数据，并允许更好地自省数据。
...
同前一节一样，对世界现状的研究(和了解)对这个小组来说是非常重要的。
为此，我们收集了目前在实践中使用的现有当前事件格式的样本。
